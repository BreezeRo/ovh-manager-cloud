class LogsInputsAddConfigureCtrl {
  ...
  initLoaders() {
    ...
    this.engine = this.CucControllerHelper.request.getHashLoader({
      loaderFunction: () => this.LogsInputsService.getDetails(this.serviceName)
        .then(details => _.find(details.engines, { engineId: this.input.data.info.engineId })),
    });
  ...
  applyConfiguration(name) {
    const helper = _.find(this.engine.data.helpers, { title: name });
